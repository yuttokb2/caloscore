...
class CaloScore(keras.Model):
    ...
    def __init__(self, num_layer, num_cond=1, name='SGM', config=None):
        ...
        self.verbose = 1  # Show progress for single process
        ...
        self.model_voxel = keras.Model(
            inputs=[inputs, inputs_time, inputs_layer, inputs_cond],
            outputs=outputs
        )
        ...
        self.model_layer = keras.Model(
            inputs=[inputs_layer, inputs_time, inputs_cond],
            outputs=outputs
        )
    ...
    def generate(self, cond):
        start = time.time()
        layer_energy = self.DDPMSampler(
            cond, self.ema_layer,
            data_shape=[self.num_layer],
            const_shape=[-1, 1]
        )

        voxels = self.DDPMSampler(
            cond, self.ema_voxel,
